...
const { generateMarkdown, scanDirectory, generateStats } = require('./generate-codebase-map.js');
...
async function convertToPdf(markdown, outputPath) {
...
async function main() {
  ...
  const outputPath = path.join(projectRoot, 'CODEBASE-MAP.pdf');
  ...
  try {
    ...
    await convertToPdf(markdown, outputPath);
    
    console.log(`\n🎉 Готово! Карта кодовой базы экспортирована в PDF: ${outputPath}`);
...
// Функция для форматирования размера файла (копируем из generate-codebase-map.js)
